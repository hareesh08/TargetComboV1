...
internal static class ExtractUrlByCountry
{
    // Default country TLD mapping
    private static readonly Dictionary<string, List<string>> DefaultCountryTlds = new()
    {
    ...
    public static void ProcessFiles(ref int totalLinesLoaded, ref int totalLinesSaved)
    {
        ...
        var outputDirectory = Path.Combine(exeDirectory, $"UrlByCountry_{timestamp}");
    ...
    private static Dictionary<string, List<string>> GetCountryTldsFromUser()
    {
        ...
        else
        {
            ...
            var countryCodes = Console.ReadLine();
            ...
            {
                var tld = tlds[i].Trim();
                var countryCode = countries[i].Trim().ToUpper();

                if (!countryTlds.ContainsKey(countryCode)) countryTlds[countryCode] = new List<string>();
    ...
    {
        var regex = new Regex(@"\.([a-z]{2,3}(?:\.[a-z]{2})?)\b", RegexOptions.IgnoreCase);
    ...
    private static void SaveCredentials(string username, string password, string tld, string outputDirectory,
        ref int totalLinesSaved)
    {
        var subfolderPath = Path.Combine(outputDirectory, tld.TrimStart('.'));
        Directory.CreateDirectory(subfolderPath); // Create subfolder for TLD if it doesn't exist

        var outputFile = Path.Combine(subfolderPath, $"{tld.TrimStart('.')}.txt");
